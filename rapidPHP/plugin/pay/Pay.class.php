...
class Pay
{
    ...
    public static function getWeiXinNotify(Request $request, Api $api, &$data = null)
    {
        ...
        $payTime = Cal()->getDate(Cal()->dateToTime($payTime));
    ...
    {
        $payId = $request->post('out_trade_no');//订单号
        if (empty($payId)) throw new Exception('数据错误 p!');

        $money = $request->post('buyer_pay_amount');//实际支付金额
        $totalAmount = $request->post('total_amount');//总金额

        $payee = $request->post('seller_email');//收款人id
        if (empty($payee)) throw new Exception('数据错误 pa!');

        $payment = $request->post('buyer_logon_id');//支付人id
        if (empty($payment)) throw new Exception('数据错误 b!');

        $state = $request->post('trade_status');//返回结果
        if (empty($state)) throw new Exception('数据错误 s!');

        $payTime = $request->post('gmt_payment');//付款时间
        if (empty($payTime)) throw new Exception('数据错误 g!');

        if (empty($request->post('sign'))) throw new Exception('数据错误 si');

<?php

namespace apps\app\classier\model;

use Exception;
use rapidPHP\modules\core\classier\Model;

/**
 * 用户主页
 * @table user_homepage
 * rapidPHP auto generate Model 2020-12-13 00:03:18
 */
class UserHomepageModel extends Model
{
    
    /**
     * table name
     */
    const NAME = 'user_homepage';
        
    
    /**
     * 主键
     * @length 
     * @typed bigint
     */
    public $id;    
    
    /**
     * 用户id
     * @length 
     * @typed bigint
     */
    public $user_id;    
    
    /**
     * 背景图文件id
     * @length 
     * @typed bigint
     */
    public $bg_fid;    
    
    /**
     * 好友数量
     * @length 
     * @typed int
     */
    public $friends_count;    
    
    /**
     * 粉丝数量
     * @length 
     * @typed int
     */
    public $follower_count;    
    
    /**
     * 关注数量
     * @length 
     * @typed int
     */
    public $following_count;    
    
    /**
     * 作品数量
     * @length 
     * @typed int
     */
    public $work_count;    
    
    /**
     * 是否删除
     * @length 
     * @typed bit
     */
    public $is_delete;    
    
    /**
     * 创建人Id
     * @length 
     * @typed bigint
     */
    public $created_id;    
    
    /**
     * 创建时间
     * @length 
     * @typed datetime
     */
    public $created_time;    
    
    /**
     * 修改人Id
     * @length 
     * @typed bigint
     */
    public $updated_id;    
    
    /**
     * 修改时间
     * @length 
     * @typed datetime
     */
    public $updated_time;    
    /**
     * 获取 主键
     * @return mixed
     */
    public function getId()
    {
        return $this->id;
    }
    
    /**
     * 设置 主键
     * @param $id
     */
    public function setId($id)
    {
        $this->id = $id;
    }
    
    /**
     * 效验 主键
     * @param string $msg
     * @throws Exception
     */
    public function validId(string $msg = 'id Cannot be empty!')
    {
        if(empty($this->id)) throw new Exception($msg);
    }
    
    /**
     * 获取 用户id
     * @return mixed
     */
    public function getUserId()
    {
        return $this->user_id;
    }
    
    /**
     * 设置 用户id
     * @param $user_id
     */
    public function setUserId($user_id)
    {
        $this->user_id = $user_id;
    }
    
    /**
     * 效验 用户id
     * @param string $msg
     * @throws Exception
     */
    public function validUserId(string $msg = 'user_id Cannot be empty!')
    {
        if(empty($this->user_id)) throw new Exception($msg);
    }
    
    /**
     * 获取 背景图文件id
     * @return mixed
     */
    public function getBgFid()
    {
        return $this->bg_fid;
    }
    
    /**
     * 设置 背景图文件id
     * @param $bg_fid
     */
    public function setBgFid($bg_fid)
    {
        $this->bg_fid = $bg_fid;
    }
    
    /**
     * 效验 背景图文件id
     * @param string $msg
     * @throws Exception
     */
    public function validBgFid(string $msg = 'bg_fid Cannot be empty!')
    {
        if(empty($this->bg_fid)) throw new Exception($msg);
    }
    
    /**
     * 获取 好友数量
     * @return int
     */
    public function getFriendsCount(): ?int
    {
        return $this->friends_count;
    }
    
    /**
     * 设置 好友数量
     * @param int|null $friends_count
     */
    public function setFriendsCount(?int $friends_count)
    {
        $this->friends_count = $friends_count;
    }
    
    /**
     * 效验 好友数量
     * @param string $msg
     * @throws Exception
     */
    public function validFriendsCount(string $msg = 'friends_count Cannot be empty!')
    {
        if(empty($this->friends_count)) throw new Exception($msg);
    }
    
    /**
     * 获取 粉丝数量
     * @return int
     */
    public function getFollowerCount(): ?int
    {
        return $this->follower_count;
    }
    
    /**
     * 设置 粉丝数量
     * @param int|null $follower_count
     */
    public function setFollowerCount(?int $follower_count)
    {
        $this->follower_count = $follower_count;
    }
    
    /**
     * 效验 粉丝数量
     * @param string $msg
     * @throws Exception
     */
    public function validFollowerCount(string $msg = 'follower_count Cannot be empty!')
    {
        if(empty($this->follower_count)) throw new Exception($msg);
    }
    
    /**
     * 获取 关注数量
     * @return int
     */
    public function getFollowingCount(): ?int
    {
        return $this->following_count;
    }
    
    /**
     * 设置 关注数量
     * @param int|null $following_count
     */
    public function setFollowingCount(?int $following_count)
    {
        $this->following_count = $following_count;
    }
    
    /**
     * 效验 关注数量
     * @param string $msg
     * @throws Exception
     */
    public function validFollowingCount(string $msg = 'following_count Cannot be empty!')
    {
        if(empty($this->following_count)) throw new Exception($msg);
    }
    
    /**
     * 获取 作品数量
     * @return int
     */
    public function getWorkCount(): ?int
    {
        return $this->work_count;
    }
    
    /**
     * 设置 作品数量
     * @param int|null $work_count
     */
    public function setWorkCount(?int $work_count)
    {
        $this->work_count = $work_count;
    }
    
    /**
     * 效验 作品数量
     * @param string $msg
     * @throws Exception
     */
    public function validWorkCount(string $msg = 'work_count Cannot be empty!')
    {
        if(empty($this->work_count)) throw new Exception($msg);
    }
    
    /**
     * 获取 是否删除
     * @return bool
     */
    public function getIsDelete(): ?bool
    {
        return $this->is_delete;
    }
    
    /**
     * 设置 是否删除
     * @param bool|null $is_delete
     */
    public function setIsDelete(?bool $is_delete)
    {
        $this->is_delete = $is_delete;
    }
    
    /**
     * 效验 是否删除
     * @param string $msg
     * @throws Exception
     */
    public function validIsDelete(string $msg = 'is_delete Cannot be empty!')
    {
        if(empty($this->is_delete)) throw new Exception($msg);
    }
    
    /**
     * 获取 创建人Id
     * @return mixed
     */
    public function getCreatedId()
    {
        return $this->created_id;
    }
    
    /**
     * 设置 创建人Id
     * @param $created_id
     */
    public function setCreatedId($created_id)
    {
        $this->created_id = $created_id;
    }
    
    /**
     * 效验 创建人Id
     * @param string $msg
     * @throws Exception
     */
    public function validCreatedId(string $msg = 'created_id Cannot be empty!')
    {
        if(empty($this->created_id)) throw new Exception($msg);
    }
    
    /**
     * 获取 创建时间
     * @return string
     */
    public function getCreatedTime(): ?string
    {
        return $this->created_time;
    }
    
    /**
     * 设置 创建时间
     * @param string|null $created_time
     */
    public function setCreatedTime(?string $created_time)
    {
        $this->created_time = $created_time;
    }
    
    /**
     * 效验 创建时间
     * @param string $msg
     * @throws Exception
     */
    public function validCreatedTime(string $msg = 'created_time Cannot be empty!')
    {
        if(empty($this->created_time)) throw new Exception($msg);
    }
    
    /**
     * 获取 修改人Id
     * @return mixed
     */
    public function getUpdatedId()
    {
        return $this->updated_id;
    }
    
    /**
     * 设置 修改人Id
     * @param $updated_id
     */
    public function setUpdatedId($updated_id)
    {
        $this->updated_id = $updated_id;
    }
    
    /**
     * 效验 修改人Id
     * @param string $msg
     * @throws Exception
     */
    public function validUpdatedId(string $msg = 'updated_id Cannot be empty!')
    {
        if(empty($this->updated_id)) throw new Exception($msg);
    }
    
    /**
     * 获取 修改时间
     * @return string
     */
    public function getUpdatedTime(): ?string
    {
        return $this->updated_time;
    }
    
    /**
     * 设置 修改时间
     * @param string|null $updated_time
     */
    public function setUpdatedTime(?string $updated_time)
    {
        $this->updated_time = $updated_time;
    }
    
    /**
     * 效验 修改时间
     * @param string $msg
     * @throws Exception
     */
    public function validUpdatedTime(string $msg = 'updated_time Cannot be empty!')
    {
        if(empty($this->updated_time)) throw new Exception($msg);
    }
}
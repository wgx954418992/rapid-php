...
class AdminUploadInterceptor extends Interceptor
{
...
//        'admin/file/upload',
    ...
    public function onHandler(Router $router, Action $action, Route $route, &$pathVariable, &$realPath, &$role)
    {
        ...
        $token = $request->request(TokenKey::ADMIN);

        if (!$token) throw new Exception('Authority fail! ', 403);

        $this->webContext->supportsParameter([UserContext::class => new UserContext($admin->getId())]);
    }
}

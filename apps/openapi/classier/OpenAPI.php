...
class OpenAPI
{
     ...
     * @throws Exception
     */
    public function transformation(array $routes, &$paths = [], &$components = [])
    {
        ...
        foreach ($classGroup as $className => $group) {
            ...
            try {
                ...
                foreach ($group as $route) {
                    ...
                    if (in_array($action->getTyped(), $this->acceptTyped) === false) continue;
                    ...
                    $paths[$action->getRoute()][$action->getRequestMethod()] = array_merge([
                        ...
                        'operationId' => $className . '::' . $route->getMethodName(),
